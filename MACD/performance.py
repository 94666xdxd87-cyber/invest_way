...
RISK_FREE_RATE_ANNUAL = 0.017   # 無風險利率（台灣定存年利率 1.7%）
...
def calc_irr(trades: list, closed_only: bool = False) -> float | None:
    """
    以現金流折現法求解年化 IRR，回傳百分比數值（例如 12.34）。
...
def calc_performance(
    ...
    if len(daily_asset) > 1:
        daily_returns  = np.diff(daily_asset) / daily_asset[:-1]
        ...
        if downside_std > 0:
            ...
            sortino_ratio = round((mean_excess / downside_std) * np.sqrt(252), 4)
